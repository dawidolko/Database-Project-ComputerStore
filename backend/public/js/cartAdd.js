...
function displayCartItems() {
    ...
    if (!cartTableBody) {
        // Jeśli nie ma elementu .cart__table_tr, to nic nie rób.
        return;
    }

    cartTableBody.innerHTML = ""; // Wyczyść aktualną zawartość tabeli
    let total = 0; // Zainicjuj całkowitą wartość koszyka jako 0

    cart.forEach((product, index) => {
        total += product.price * product.quantity; // Dodaj wartość produktu do całkowitej wartości koszyka

        // Utwórz wiersz tabeli dla produktu
        const tr = document.createElement("tr");

        // Dodaj komórki do wiersza tabeli
        tr.innerHTML = `
            <td><img src="${product.image}" alt="${
            product.name
        }" width="50" height="50"></td>
